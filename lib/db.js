...
// Records the Resend email id for a freshly-sent invoice email and
// resets opened_at so the "opened" notification fires again for this send.
export async function updateEmailSent(id, resendEmailId) {
  const db = getClient()
  await db.execute({
    sql: 'UPDATE invoices SET last_emailed = ?, resend_email_id = ?, opened_at = NULL WHERE id = ?',
    args: [new Date().toISOString(), resendEmailId || null, id],
  })
}

export async function getInvoiceByResendEmailId(resendEmailId) {
  const db = getClient()
  const result = await db.execute({
    sql: 'SELECT * FROM invoices WHERE resend_email_id = ?',
    args: [resendEmailId],
  })
  return rowToInvoice(result.rows[0])
}

// Marks the invoice as opened; returns false if it was already marked
// (so callers only send one "opened" notification per send).
export async function markInvoiceOpened(resendEmailId) {
  const db = getClient()
  const result = await db.execute({
    sql: `UPDATE invoices SET opened_at = ? WHERE resend_email_id = ? AND opened_at IS NULL`,
    args: [new Date().toISOString(), resendEmailId],
  })
